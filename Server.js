...
const requirePermission = (req, res, next) => {
    if (req.session && req.session.user) {
        if (req.path === '/userManager' || req.path === '/userAPI') {
            if (req.session.user.userLogin.permission != 'ADMIN') {
                return res.sendStatus(403);// Không cho phép truy cập vào trang, trả về mã lỗi 403
            }
            return next();
        }
...
app.use(function (req, res, next) {
    if (req.path === '/' || req.path === "/login" || req.path === "/register") {
        ...
        } // Cho phép truy cập vào trang đăng nhập và đăng xuất
        return next();
    } else {
        return requirePermission(req, res, next); // Kiểm tra quyền truy cập của người dùng
    }
});
...
app.post('/editUserLogin', async (req, res, next) => {
    ...
    if (check) {
        ...
        await userUpdate.updateOne({ name: name, email: email, password: pass, img: imgUpdate, permission: userEdit.permission, status: true });
